var PLEX = {
	...
	display_genre_list: function(genres) {
		...
		if(genres.length > 0) {
			...
			var list_html = '<li data-genre="all"><em>'+PLEX.current_section.num_items+'</em>Alle</li>';
			...
			if(num_hidden>0) {
				list_html += '<li id="genre_show_all">Zeige '+num_hidden+' mehr...</li>';
				list_html += '<li id="genre_hide_all">Zeige weniger...</li>';
			}
	...
	display_director_list: function(total_count, directors) {
		...
		var num_to_show_before_hiding = 5;
		var count = num_hidden = 0;
		var list_html = '<li data-director="all"><em>'+total_count+'</em>Alle</li>';
		...
		if(num_hidden>0) {
			list_html += '<li id="director_show_all">Zeige '+num_hidden+' mehr...</li>';
			list_html += '<li id="director_hide_all">Zeige weniger...</li>';
		}
	...
	display_seen_list: function(seen) {
		...
		var list_html = '<li data-seen="all"><em>'+PLEX.current_section.num_items+'</em>Alle</li>';
		...
		list_html += '<li data-seen="true"><em>'+seen_count+'</em>gesehen</li>';
		...
		list_html += '<li data-seen="false"><em>'+unseen_count+'</em>Nicht gesehen</li>';
	...
	display_items: function(items) {
		...
		if(num_items==0) {
			PLEX._section_meta.text("Keine Einträge in der Sammlung");
			PLEX._item_list_status.html("<p>Hier sind keine Einträge vorhanden.</p>").show();
		} else {
			PLEX._item_list_status.hide();
			//PLEX._section_meta.text(number_format(num_items)+" "+inflect(num_items,"item")+" in this collection");
			PLEX._section_meta.text(number_format(num_items)+" "+"Einträge"+" in dieser Sammlung");
		}
	...
	generate_item_content: function() {
		...
		var popup_header = '<div id="popup-header"><p class="right"><span class="popup-close">Schließen</span></p><p>Mediathek &raquo; '+PLEX.current_section.title+' &raquo; '+PLEX.current_item.title+'</p></div>';
		...
		if(PLEX.current_item.duration > 0) {
			...
			//popup_sidebar_meta += '<li>Länge: '+minutes+' '+inflect(minutes,'minute')+'</li>';
			popup_sidebar_meta += '<li>Länge: '+minutes+' '+'Minuten'+'</li>';
		}
		if(PLEX.current_item.studio != false) popup_sidebar_meta += '<li>Studio: '+PLEX.current_item.studio+'</li>';
		if(PLEX.current_item.release_year != false) popup_sidebar_meta += '<li>veröffentlicht: '+PLEX.current_item.release_year+'</li>';
		if(PLEX.current_item.content_rating != false) popup_sidebar_meta += '<li>Wertung: '+PLEX.current_item.content_rating+'</li>';
		if(PLEX.current_item.num_seasons > 0) popup_sidebar_meta += '<li>Staffeln: '+PLEX.current_item.num_seasons+'</li>';
		if(PLEX.current_item.num_episodes > 0) popup_sidebar_meta += '<li>Episoden: '+PLEX.current_item.num_episodes+'</li>';
		...
		if(
			...
			if(PLEX.current_item.director) popup_content += '<li><strong>Regie:</strong> '+PLEX.current_item.director.join(", ")+'</li>';
			if(PLEX.current_item.role) popup_content += '<li><strong>Darsteller:</strong> '+PLEX.current_item.role.join(", ")+'</li>';
			...
			if(PLEX.current_item.media) {
				...
				popup_content += '<li><strong>Video:</strong> codec: '+media.video_codec+', Bildrate: '+media.video_framerate+ ((media.video_resolution != undefined && media.video_resolution>0)?', Bildhöhe: '+media.video_resolution:'') + ((media.aspect_ratio != undefined && media.aspect_ratio>0)?', Seitenverhältnis: '+media.aspect_ratio:'') +'</li>';
				popup_content += '<li><strong>Audio:</strong> codec: '+media.audio_codec+', Kanäle: '+media.audio_channels+'</li>';
				if(media.total_size != false) popup_content += '<li><strong>Datei:</strong> '+hl_bytes_to_human(media.total_size)+' @ '+media.bitrate+'bps</li>';
			}
			popup_content += '</ul>';
		}
		...
		if(PLEX.current_item.num_seasons && PLEX.current_item.num_seasons>0) {
			popup_content += '<div id="popup_seasons"><h4>Staffel Browser</h4><table><tr><td id="popup_seasons_seasons"><ul>';
			...
			$("#popup_seasons_episodes li").live("click", function(){
				...
				var html = '<h5>'+episode.title+'</h5><p class="meta">'+episode_tag(season,episode)+' | '+minutes+' '+'Minuten'+' | Wertung: '+episode.rating+'</p><p>'+episode.summary+'</p>';
